...
class PluginBuilder
{
    ...
    private string $pluginName = 'tsml-for-unity';
    ...
    {
        $mainFile = $this->pluginDir . DIRECTORY_SEPARATOR . 'tsml-for-unity.php';
        if (file_exists($mainFile)) {
            $content = file_get_contents($mainFile);
            if (preg_match('/Version:\s*([0-9.]+\s*\w*)/', $content, $matches)) {
                return trim($matches[1]);
            }
            if (preg_match("/define\s*\(\s*'TSML_FOR_UNITY_VERSION'\s*,\s*'([^']+)'/", $content, $matches)) {
                return $matches[1];
            }
        }
        return '0.0.1';
     ...
     */
    private function runComposer(string $type = 'production'): void
    {
        ...
        if ($returnCode !== 0) {
            $this->error("Composer install failed with exit code: {$returnCode}");
    ...
    private function createZip(string $archiveName, array $excludes): void
    {
        ...
        foreach ($files as $file) {
            ...
            // Add directory prefix so the plugin extracts into its own folder
...
TSML for Unity WordPress Plugin Build Script
============================================
Platform: {$platform}
...
Composer Scripts:
  composer build                          # Build production archive
  composer build:production               # Build production archive
  composer build:dev                      # Build development archive
  composer build:clean                    # Clean build directory
...
Testing Commands:
  ...
  composer check                          # Run cs, stan, and test
...
Note:
  TSML for Unity is a companion plugin to Unity. Make sure Unity is installed
  and activated before activating TSML for Unity.
